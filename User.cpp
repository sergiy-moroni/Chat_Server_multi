...
void const User::SetName(const std::string& name) 
{
	_name = name;
}
const std::string User::Getpass() 
{
	return _pass;
}
const std::string User::Getlog() 
{
	return _login;
}
...
//получение сообщения 
void const User::Setmessage(const Message& message)
{
	_messages.push_back(message);
}

// чтение сообщений 
void const  User::PrintMessages()
{
	std::cout << std::endl;
